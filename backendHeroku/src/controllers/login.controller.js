...
loginController.retrieveCredentials = async (req, res) => {
  ...
  console.log(
    "Contraseña ingresada: %s, real: %s",
    req.params.contrasenna,
    funcionarioEncontrado.contrasenna
  );
  ...
  const token = jwt.sign(
    { identificacion: funcionarioEncontrado.identificacion },
    "secretkey"
  );
  ...
  res
    ...
    .json({
      token,
      identificacion: funcionarioEncontrado.identificacion,
      admin: funcionarioEncontrado.admin,
      jefatura: funcionarioEncontrado.jefatura,
      nombre_completo: funcionarioEncontrado.nombre_completo,
      campus_departamento_jefatura: funcionarioEncontrado.campus_departamento_jefatura,
      operador: funcionarioEncontrado.tipo_funcionario == "Operador" ? 1 : 0,
      idParqueoOperador: funcionarioEncontrado.idParqueoOperador,
    });
};
